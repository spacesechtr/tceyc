...
date=date2.astimezone(pytz.timezone("Asia"))
date=str(date)
date=date[0:10]
...
def truecaller_search(token, num):
    ...
    turl = "https://search5-noneu.truecaller.com/v2/search?q=" + num + "&countryCode=PK&type=4&locAddr=&placement=SEARCHRESULTS%2CHISTORY%2CDETAILS&encoding=json"
    theaders = {
        "user-agent": "Truecaller/11.5.7 (Android;10)",
        "Accept-Encoding": "gzip",
        "authorization": token,
...
def eyecon_search(num):
    url = "https://api.eyecon-app.com/app/getnames.jsp?cli=92" + num + "&lang=en&is_callerid=true&is_ic=true&cv=vc_312_vn_2.0.312_a&requestApi=URLconnection&source=Other"
    headers = {
...
def fb_search(num):
    fburl = "https://api.eyecon-app.com/app/pic?cli=92" + num + "&is_callerid=true&size=big&type=0&cancelfresh=0&cv=vc_312_vn_2.0.312_a"
    fbheaders = {
        "User-Agent": "Dalvik/2.1.0 (Linux; U; Android 10; GM1903 Build/QKQ1.190716.003)",
        "Accept": "application/json",
        "Accept-Encoding": "gzip",
